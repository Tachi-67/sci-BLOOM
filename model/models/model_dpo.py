...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, beta=1.0, **kwargs):
        """
    ...
    def _init_weights(self, **kwargs):
        ...
        init_strategy = kwargs.get('custom_module_init_strategy', 'random')
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        chosen_texts = [prompt + chosen for prompt, chosen in zip(batch["prompt"], batch["chosen"])]
        rejected_texts = [prompt + rejected for prompt, rejected in zip(batch["prompt"], batch["rejected"])]
    ...
    def prediction_step_reward(
        ...
        #scaled ratios
        chosen_rewards = beta * chosen_ratio #.detach() #check if detach is needed
